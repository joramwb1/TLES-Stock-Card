...
const STAFF_PASS = "107979tles"; // ⚠️ Consider moving this to server-side config
...
function updateUI() {
    ...
    const filterCat = getEl('filter-category')?.value || "All";
    ...
    inventory.forEach(item => {
        ...
        if (matchesCat && matchesSearch) {
            ...
            const minVal = parseInt(item.min) || 5;
            const isLow = qtyVal <= minVal;
            html += `
                <tr>
                    ...
                    <td><span class="qty-pill ${isLow ? 'qty-low' : ''}">${qtyVal} ${item.unit}</span></td>
...
function sortTable(key) {
    ...
    inventory.sort((a, b) => {
        if (key === 'qty') {
            const aq = parseInt(a.qty) || 0;
            const bq = parseInt(b.qty) || 0;
            return (aq - bq) * sortDir[key];
        }
...
function restockItem(id) {
    ...
    if (addQty && !isNaN(addQty)) {
        window.fb.update(window.fb.ref(window.fb.db, 'inventory/' + id), { qty: (parseInt(item.qty) || 0) + parseInt(addQty) });
